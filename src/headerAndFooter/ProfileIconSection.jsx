...
export default function ProfileIconSection({ onProfileIconChange, location }) {
    ...
    return (
        <div>
            <div className='flex bg-white/80 lg:rounded-t-xl mb-1'>
                <h1 className='p-5 text-xl text-black font-bold'>
                    {location.state.user}
                </h1>
                <div className='size-20' />
            </div>
            <div className='flex flex-col gap-5 pt-5 pb-5 px-2'>
                <div className='text-center bg-white/70 border-1 rounded-xl mx-5'>
                ...
                <div className='flex flex-col gap-5 items-center'>
                    <button className='w-[90%] border-1 p-1 rounded-xl cursor-pointer bg-white text-lg text-black hover:bg-gray-500 transition-all duration-500 shadow-xl' onClick={() => handleLogout()}>Logout</button>
                </div>
            </div>
        </div>
    );
